	...
	static verifyHeaders(
		...
		if (!contentType) {
			return res
				...
				.send(
					`${resCodes.BAD_GATEWAY.prefix}: Missing Content-Type Header In Gateway`,
				);
		}
		if (contentType !== 'application/json') {
			return res
				.status(resCodes.BAD_GATEWAY.code)
				.send(
					`${resCodes.BAD_GATEWAY.prefix}: Invalid Content-Type Header In Gateway`,
				);
		}
		...
		if (!apiKeyHeader) {
			return res
				.status(resCodes.BAD_GATEWAY.code)
				.send(
					`${resCodes.BAD_GATEWAY.prefix}: Missing API Key In Gateway`,
				);
		}
		...
		if (typeof apiKeyHeader !== 'string') {
			return res
				.status(resCodes.BAD_GATEWAY.code)
				.send(
					`${resCodes.BAD_GATEWAY.prefix}: Invalid API Key In Gateway`,
				);
		}
		...
		try {
			...
			if (!apiKeySecret) {
				return res
					...
					.send(
						`${resCodes.INTERNAL_SERVER.prefix}: No API Key In Back-End`,
					);
			}
			...
			if (apiKeyHeader !== apiKeySecret) {
				return res
					.status(resCodes.BAD_GATEWAY.code)
					.send(
						`${resCodes.BAD_GATEWAY.prefix}: Unauthorized API Key`,
					);
			}
			return next();
		} catch (err) {
